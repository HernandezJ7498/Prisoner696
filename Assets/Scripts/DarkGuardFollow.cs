...
public class DarkGuardFollow : MonoBehaviour {

	// Use this for initialization
	public GameObject ThePlayer;
	public float TargetDistance;
	public float allowedRange = 95;
	...
	void Start () {
		TheEnemy = GameObject.FindWithTag ("DarkGuard");
		...
		if(Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out shot)){
			...
			if(TargetDistance < allowedRange){
				...
				if(AttackTrigger == 0){
					...
					RandomNumber = Random.Range (0,375);
					if (RandomNumber == 3) {
						transform.position = Vector3.MoveTowards (transform.position, ThePlayer.transform.position, 2f);
					}
